...
export const useBases = defineStore('basesStore', () => {
  ...
  const activeProjectId = computed(() => {
    ...
    // Don't set active project for workspace pages
    const workspaceRoutes = ['integrations', 'product-matcher', 'product', 'settings', 'feed']
    if (workspaceRoutes.some(routeName => route.value.name === `index-typeOrId-${routeName}`)) {
      return undefined
    }
  ...
  const showProjectList = computed(() => {
    // Don't show project list for base routes
    if (route.value.params.typeOrId === 'base') {
      return false
    }
    
    // Don't show project list for workspace pages
    const workspaceRoutes = ['integrations', 'product-matcher', 'product', 'settings', 'feed']
    if (workspaceRoutes.some(routeName => route.value.name === `index-typeOrId-${routeName}`)) {
      return false
    }
    
    // Show project list when no baseId is present (but not for workspace routes)
    return !route.value.params.baseId
  })
