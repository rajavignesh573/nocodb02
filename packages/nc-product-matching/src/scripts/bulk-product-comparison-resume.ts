...
class BulkProductComparisonResume {
  ...
  private async loadInternalProducts(): Promise<InternalProduct[]> {
    ...
    while (hasMore) {
      const batch = await DatabaseHelper.loadInternalProducts(batchSize, offset);
  ...
  private async loadExternalProducts(): Promise<ExternalProduct[]> {
    ...
    while (hasMore) {
      const batch = await DatabaseHelper.loadExternalProducts(batchSize, offset);
      allProducts.push(...batch);
   ...
   */
  private async initializeCSVWriter(): Promise<void> {
   ...
   */
  private async writeResultToCSV(result: ComparisonResult): Promise<void> {
    try {
