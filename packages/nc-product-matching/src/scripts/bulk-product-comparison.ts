...
interface ComparisonResult {
  internal_id: string;  // Added unique identifier for resume capability
  external_id: string;  // Added unique identifier for resume capability
  ...
  processed_at: string;  // Added timestamp for tracking
}
...
class BulkProductComparison {
  ...
  private readonly MINIMUM_MATCH_CONFIDENCE = 50.0; // Only log matches above this threshold
  private csvWriter: any;
  private csvPath: string;
  private processedInternalIds = new Set<string>();  // Track processed products for resume
  private readonly CHECKPOINT_FILE = 'comparison_checkpoint.json';

  constructor(
    private useDatabase: boolean = true, // Default to database now
    private batchSize: number = 1000 // Process in batches for large datasets
  ) {
  ...
  async run(): Promise<void> {
    try {
      ...
      if (this.useDatabase) {
        ...
        const stats = await DatabaseHelper.getStatistics();
  ...
  private async loadInternalProducts(): Promise<InternalProduct[]> {
    if (this.useDatabase) {
      ...
      const allProducts: InternalProduct[] = [];
      let offset = 0;
      let hasMore = true;
      
      while (hasMore) {
        const batch = await DatabaseHelper.loadInternalProducts(
          this.batchSize,
          offset
        );
  ...
  private async loadExternalProducts(): Promise<ExternalProduct[]> {
    if (this.useDatabase) {
      ...
      while (hasMore) {
        const batch = await DatabaseHelper.loadExternalProducts(
          this.batchSize,
          offset
        );
...
if (require.main === module) {
  ...
  console.log(
    useDatabase ? '💾 Running with REAL DATABASE data...' : 
