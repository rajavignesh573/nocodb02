 ...
 */
export class FuzzyMatchingHelper {
  private static readonly stemmer = natural.PorterStemmer;
  private static readonly tokenizer = new natural.WordTokenizer();
  ...
  static calculateNameSimilarity(name1: string, name2: string): number {
    ...
    let weightedScore = similarities.reduce((sum, score, index) => 
  ...
  static calculateBrandSimilarity(brand1?: string, brand2?: string): number {
    ...
    if (maxSimilarity >= 0.95) return maxSimilarity * 0.8;  // Reduced from 0.9
    if (maxSimilarity >= 0.9) return maxSimilarity * 0.6;   // Reduced from 0.7
    if (maxSimilarity >= 0.85) return maxSimilarity * 0.4;  // Reduced from 0.5
  ...
  static calculateCategorySimilarity(category1?: string, category2?: string): number {
    if (!category1 || !category2) return 0.0; // No default similarity - must have both
    ...
    if (similarity >= 0.9) return similarity * 0.8;  // Reduced score even for high similarity
    if (similarity >= 0.8) return similarity * 0.6;  // Further reduced
   ...
   */
  private static getCategoryMappedSimilarity(category1: string, category2: string): number {
    // This is a simplified version - the actual implementation will query the database
    // For now, implement common mappings directly
    
    const categoryMappings: { [key: string]: { [key: string]: number } } = {
   ...
   */
  private static getJaccardSimilarity(str1: string, str2: string): number {
   ...
   */
  private static getCosineSimilarity(str1: string, str2: string): number {
   ...
   */
  private static getLevenshteinSimilarity(str1: string, str2: string): number {
   ...
   */
  private static getDiceSimilarity(str1: string, str2: string): number {
   ...
   */
  private static extractKeyProductTerms(name: string): string[] {
   ...
   */
  private static calculateKeyTermOverlap(terms1: string[], terms2: string[]): number {
  ...
  private static calculatePriceSimilarity(price1?: number, price2?: number): number {
    ...
    if (percentageDiff <= 2) return 1.0;
    ...
    if (percentageDiff <= 30) return 0.5;
    if (percentageDiff <= 40) return 0.3;
    return 0.1;
  }
   ...
   */
  private static calculateGTINSimilarity(gtin1?: string, gtin2?: string): number {
    if (!gtin1 || !gtin2) return 0;

    // Exact match
    if (gtin1 === gtin2) return 1.0;
