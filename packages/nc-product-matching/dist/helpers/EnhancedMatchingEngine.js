...
exports.EnhancedMatchingEngine = void 0;
const FuzzyMatchingHelper_1 = require("./FuzzyMatchingHelper");
const MatchingDebugger_1 = require("./MatchingDebugger");
/**
...
class EnhancedMatchingEngine {
     ...
     */
    static async findMatches(internalProduct, externalProducts, sourceInfo) {
        ...
        for (const extProduct of externalProducts) {
            ...
            if (!requirementsCheck.passed) {
                MatchingDebugger_1.MatchingDebugger.logDecision(internalProduct.title, extProduct.title, sourceInfo.name, 'REJECTED', requirementsCheck.reason);
                rejected++;
                continue;
            }
            ...
            if (similarity.overall < 0.30) { // Only reject truly poor matches
                MatchingDebugger_1.MatchingDebugger.logDecision(internalProduct.title, extProduct.title, sourceInfo.name, 'REJECTED', `Score too low: ${(similarity.overall * 100).toFixed(1)}% < 30%`, similarity.overall, similarity);
                rejected++;
                continue;
            }
            ...
            const candidate = this.createMatchCandidate(internalProduct, extProduct, sourceInfo, similarity, requirementsCheck.reasons);
            candidates.push(candidate);
            // Log successful match
            MatchingDebugger_1.MatchingDebugger.logDecision(internalProduct.title, extProduct.title, sourceInfo.name, 'MATCHED', `Match found (${candidate.tier} tier, ${candidate.confidence.toFixed(1)}% confidence)`, candidate.score, similarity);
        }
     ...
     */
    static checkMandatoryRequirements(internal, external) {
        const reasons = [];
     ...
     */
    static calculateEnhancedSimilarity(internal, external) {
        const subscores = {};
        const reasons = [];
        let penalties = [];
        let capsApplied = [];
        ...
        const nameResult = this.calculateNameScore(internal.title || '', external.title || '');
        subscores.name = nameResult.score;
        reasons.push(...nameResult.reasons);
        penalties.push(...nameResult.penalties);
        // Brand Score (20% weight)
        const brandResult = this.calculateBrandScore(internal.moombs_brand || internal.brand || '', external.brand || '');
        ...
        const categoryResult = this.calculateCategoryScore(internal.moombs_category || internal.category_id || '', external.category_id || '');
        ...
        const priceResult = this.calculatePriceScore(internal.price, external.price, external.discount ? true : false // Detect if on sale
        );
        subscores.price = priceResult.score;
        reasons.push(...priceResult.reasons);
        // Calculate weighted final score
        let finalScore = (subscores.name * this.CONFIG.WEIGHTS.name +
            subscores.brand * this.CONFIG.WEIGHTS.brand +
            subscores.category * this.CONFIG.WEIGHTS.category +
            subscores.price * this.CONFIG.WEIGHTS.price) / 100; // Convert to 0-1 scale
     ...
     */
    static calculateGtinMatch(gtin1, gtin2) {
        if (!gtin1 || !gtin2) {
     ...
     */
    static calculateNameScore(name1, name2) {
        if (!name1 || !name2) {
            return { score: 0, reasons: [], penalties: [], modelMismatch: false };
        }
        // Base semantic similarity (0-1)
        const semanticSim = FuzzyMatchingHelper_1.FuzzyMatchingHelper.calculateNameSimilarity(name1, name2);
     ...
     */
    static calculateBrandScore(brand1, brand2) {
        if (!brand1 || !brand2) {
        ...
        const similarity = FuzzyMatchingHelper_1.FuzzyMatchingHelper.calculateBrandSimilarity(brand1, brand2);
        if (similarity > 0.8) {
            return {
     ...
     */
    static calculateCategoryScore(cat1, cat2) {
        if (!cat1 || !cat2) {
     ...
     */
    static calculatePriceScore(price1, price2, onSale = false) {
     ...
     */
    static detectAccessoryMismatch(name1, name2) {
    ...
    static detectPackMismatch(name1, name2) {
    ...
    static detectModelMismatch(name1, name2) {
        // Simplified model detection for core baby gear
        const coreGear = ['stroller', 'car seat', 'crib', 'highchair'];
        const isCoreGear = coreGear.some(gear => name1.toLowerCase().includes(gear) || name2.toLowerCase().includes(gear));
        if (!isCoreGear)
            return false;
    ...
    static areBrandAliases(brand1, brand2) {
    ...
    static areSameBranch(cat1, cat2) {
        ...
        return branches.some(branch => branch.some(cat => cat1.includes(cat)) &&
            branch.some(cat => cat2.includes(cat)));
    }
    static areSameDepartment(cat1, cat2) {
        // Simplified department detection
        const departments = [
            ['baby', 'infant', 'toddler', 'newborn'],
            ['kids', 'children', 'child']
        ];
        return departments.some(dept => dept.some(d => cat1.includes(d)) &&
            dept.some(d => cat2.includes(d)));
    }
    /**
     * Create match candidate following ProductMatchingRules.md output contract
     */
    static createMatchCandidate(internal, external, sourceInfo, similarity, matchReasons) {
     ...
     */
    static getMatchQualitySummary(candidates) {
...
exports.EnhancedMatchingEngine = EnhancedMatchingEngine;
// ProductMatchingRules.md Configuration
EnhancedMatchingEngine.CONFIG = {
    // Tier-0: Exact GTIN Match
...
//# sourceMappingURL=EnhancedMatchingEngine.js.map