...
class ProductMatchingService {
    ...
    async getFilterOptions(context) {
        // Check cache first
        const now = Date.now();
        ...
        try {
            ...
            const products = await this.ncMeta.metaList2(context.workspace_id, 'default', 'odoo_moombs_int_product_data', { limit: 1000 } // Get enough to find all unique brands
            );
            const brands = Array.from(new Set(products
                .map((p) => p.brand)
                .filter((brand) => brand && brand.trim() !== ''))).sort();
            const categories = Array.from(new Set(products
                .map((p) => p.category)
                .filter((category) => category && category.trim() !== '' && category !== 'All'))).sort();
            // Get sources from ProductMatchSource table
            let sources = [];
            try {
                const sourcesList = await ProductMatchSource_1.default.list(context, 100, 0, this.ncMeta);
                sources = sourcesList
    ...
    async getProducts(context, filter) {
        try {
            ...
            const products = await this.ncMeta.metaList2(context.workspace_id, 'default', 'odoo_moombs_int_product_data', queryOptions);
            ...
            if (filter.q) {
                ...
                filteredProducts = filteredProducts.filter((product) => product.product_name?.toLowerCase().includes(searchTerm) ||
            ...
            if (filter.brand) {
                const beforeCount = filteredProducts.length;
                console.log(`🔍 DEBUG: Before brand filter - sample brands: ${filteredProducts.slice(0, 3).map((p) => `"${p.brand}"`).join(', ')}`);
                // Case-insensitive brand filter
                const filterBrandLower = filter.brand.toLowerCase();
                filteredProducts = filteredProducts.filter((product) => product.brand && product.brand.toLowerCase() === filterBrandLower);
                console.log(`🔍 DEBUG: Brand filter "${filter.brand}" (case-insensitive) - ${beforeCount} -> ${filteredProducts.length} products`);
            }
            if (filter.categoryId) {
                filteredProducts = filteredProducts.filter((product) => product.category === filter.categoryId);
            }
    ...
    async getProductById(context, productId) {
        try {
            ...
            const product = await this.ncMeta.metaGet2(context.workspace_id, 'default', 'odoo_moombs_int_product_data', actualId);
            if (!product)
                return null;
            return {
    ...
    async findCandidatesForSource(context, localProduct, source, filter) {
        try {
            ...
            const externalProductsFormatted = externalProducts.map((ext) => ({
            ...
            const matchCandidates = await EnhancedMatchingEngine_1.EnhancedMatchingEngine.findMatches(internalProductFormatted, externalProductsFormatted, {
                id: source.id || '',
                code: source.code || '',
                name: source.name || ''
            });
            // Get quality summary
            const qualitySummary = EnhancedMatchingEngine_1.EnhancedMatchingEngine.getMatchQualitySummary(matchCandidates);
            console.log(`📊 MATCH QUALITY SUMMARY:`, qualitySummary);
            // Convert back to expected format with backward compatibility
            const candidates = matchCandidates.map(candidate => ({
    ...
    calculateSimilarityScore(localProduct, externalProduct) {
        ...
        const result = FuzzyMatchingHelper_1.FuzzyMatchingHelper.calculateProductSimilarity({
            title: localProduct.title,
            brand: localProduct.brand,
    ...
    calculateNameSimilarity(name1, name2) {
        return FuzzyMatchingHelper_1.FuzzyMatchingHelper.calculateNameSimilarity(name1, name2);
    }
    /**
     * @deprecated Use FuzzyMatchingHelper.calculateBrandSimilarity instead
     */
    calculateBrandSimilarity(brand1, brand2) {
        return FuzzyMatchingHelper_1.FuzzyMatchingHelper.calculateBrandSimilarity(brand1, brand2);
    }
    /**
     * Check if two categories represent relevant product types
     */
    isRelevantProductMatch(internalCategory, externalCategory) {
     ...
     */
    checkBrandVariations(brand1, brand2) {
     ...
     */
    inferCategoryFromProductName(productName, externalCategory) {
     ...
     */
    async getCategorySimilarity(internalCategory, externalCategory) {
        try {
            // Query the category mapping table
            const mappingResult = await this.ncMeta.metaList2('default', // workspace_id
            'default', // base_id
            ...
            const reverseMappingResult = await this.ncMeta.metaList2('default', // workspace_id
            'default', // base_id
            'nc_product_match_category_map', {
                condition: {
    ...
    async createMatch(context, matchData, userId) {
        // Get source by code
        const source = await ProductMatchSource_1.default.getByCode(context, matchData.source_code, this.ncMeta);
        if (!source) {
            throw new Error(`Source not found: ${matchData.source_code}`);
        ...
        const existingMatch = await ProductMatch_1.default.findByProductPair(context, matchData.local_product_id, matchData.external_product_key, this.ncMeta);
        if (existingMatch) {
    ...
    async removeMatch(context, matchData, userId) {
        ...
        await ProductMatch_1.default.delete(context, existingMatch.id, this.ncMeta);
        return { success: true };
    }
    /**
     * Extract numeric ID from string format (int-123 → 123) or return as is
     */
    extractNumericId(productId) {
